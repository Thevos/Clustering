		...
		Camera* camera)
{
	...
	// Good HSV threshold values: 10, 49, 47

	// Background subtraction H
	const std::vector<cv::Mat> lol = camera->getBgHsvChannels();
	...
	dilate(foreground, dilation, element2);
	erode(dilation, erosion, element1);

	camera->setForegroundImage(erosion);
}
