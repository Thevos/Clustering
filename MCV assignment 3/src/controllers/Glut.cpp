...
void Glut::drawVoxels()
{
	...
	{
		glColor4f(voxels[v]->color[0], voxels[v]->color[1], voxels[v]->color[2], 0.5f);
...
void Glut::cluster() {
	...
	Mat bestLabels;
	Mat centers;
	//convert voxel space to 2D for kmeans
	...
	for (int x = 0; x < 4; x++) {
		...
		Mat foreground = m_Glut->getScene3d().getCameras()[x]->getForegroundImage();
		for (int i = 0; i < voxels.size(); i++) {
		...
		for (int i = 0; i < voxels.size(); i++)
		{
			if (temp[i] != '\0' && voxels[i]->valid_camera_projection[x])
			{
				...
				if (find(occlusion.begin(), occlusion.end(), tuple<Point, double>(voxels[i]->camera_projection[x], distance)) != occlusion.end())
					voxels[i]->color = foreground.at<Vec3b>(voxels[i]->camera_projection[x]);
			}
		}
	}
